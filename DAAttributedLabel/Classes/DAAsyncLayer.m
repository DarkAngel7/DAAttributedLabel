...
static dispatch_queue_t asyncDisplayQueue() {
    ...
    dispatch_once(&onceToken, ^{
        if ([UIDevice currentDevice].systemVersion.floatValue >= 8.0) {
            ...
            displayQueue = dispatch_queue_create("com.darkangel.textkit.render", attr);
        }else {
            displayQueue = dispatch_queue_create("com.darkangel.textkit.render", DISPATCH_QUEUE_CONCURRENT);
...
@implementation DAAsyncLayer {
    DASentinel *_sentinel;
}
...
- (void)displayAsync:(BOOL)asynchronously {
    __strong id<DAAsyncLayerDelegate> delegate = _asyncDelegate;
    DAAsyncLayerDisplayTask *task = [delegate newAsyncDisplayTask];
    if (task.willDisplay) {
    ...
    UIColor *backgroundColor = (opaque && self.backgroundColor) ? [UIColor colorWithCGColor:self.backgroundColor] : UIColor.clearColor;
    if (asynchronously) {
        unsigned int value = [_sentinel value];
        DASentinel *sentinel = _sentinel;
...
@implementation DAAsyncLayerDisplayTask
@end
